...
async function signIn(){
  try {
    if (window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform()) {
      if (!nativeAuth) {
        throw new Error("Le module FirebaseAuthentication n'est pas disponible dans l'APK.");
      }
...
async function loadNativeAuth(){
  ...
  try {
    ...
    if (!nativeAuth && window.Capacitor.Plugins) {
      nativeAuth = window.Capacitor.Plugins.FirebaseAuthentication || null;
    }

    if (!nativeAuth) {
      throw new Error("FirebaseAuthentication non enregistré.");
    }
  } catch (e) {
